...
| `compare.html` | Comparer / Compare | `compare` | Side-by-side comparisons by site, program, intervention type, sex, age group, and selected combined dimensions |
...
1. **`index.html`** — User drops one or more CommCare `.xlsx` export files. `parser.js` parses them client-side using SheetJS (`xlsx@0.18.5` from CDN). The parsed data object is stored locally in IndexedDB via `storage.js`.
2. **`analytics.html`**, **`profiles.html`**, **`pathways.html`**, **`compare.html`** — These pages read the locally stored dashboard data on load, and render their respective sections.

`sessionStorage` is retained only as a small-data/backward-compatibility fallback. Large datasets should use IndexedDB, which remains local to the user's browser/profile.

### Stored dashboard data shape
...
- **`js/storage.js`** — Local-only data persistence using IndexedDB. Entry points: `saveDashboardData(data)`, `loadDashboardData()`, `clearDashboardData()`. Requests persistent browser storage when available and falls back to `sessionStorage` only when IndexedDB is unavailable.
...
- **`js/compare.js`** — Side-by-side comparison page logic. Loads the same locally stored dashboard data, reuses `getFilters()` and `getFilteredEpisodes()`, groups filtered episodes by the selected dimension, computes comparison metrics, renders the comparison table/charts, and exports comparison rows as CSV.
...
`analytics.js` runs async `init()` on `DOMContentLoaded`. `init()`:
1. Loads data from IndexedDB via `loadDashboardData()`
...
- `.no-data-banner` — full-page empty state shown when no local dashboard data is available
...
- `.Codex/` is in `.gitignore` — do not commit it.
